...
if ! python3 -c "import whisperx" 2>/dev/null; then
    echo "Error: WhisperX is not installed or not accessible to Python."
    echo ""
    echo "WhisperX must be installed as a Python module (NOT with pipx)."
    echo ""
    echo "Install it with one of these methods:"
    echo "  1. Using pip with --user (recommended):"
    echo "     sudo apt install ffmpeg"
    echo "     pip3 install --user whisperx"
    echo ""
    echo "  2. Using --break-system-packages:"
    echo "     pip3 install --break-system-packages whisperx"
    echo ""
    echo "  3. Using virtual environment:"
    echo "     python3 -m venv ~/whisperx-env"
    echo "     source ~/whisperx-env/bin/activate"
    echo "     pip install whisperx"
    echo "     # Then run this script from within the venv"
    echo ""
    echo "If you installed with pipx, uninstall it first:"
    echo "     pipx uninstall whisperx"
    echo ""
    echo "Required: HuggingFace token (https://huggingface.co/settings/tokens)"
...
if [ $EXIT_CODE -eq 0 ]; then
    echo ""
    echo "Output files:"
    echo "  - ${FILENAME_NO_EXT}_speakers.txt (readable transcript)"
    echo "  - ${FILENAME_NO_EXT}_speakers.srt (subtitles with speakers)"
    echo "  - ${FILENAME_NO_EXT}_speakers.json (detailed data)"
else
    echo ""
